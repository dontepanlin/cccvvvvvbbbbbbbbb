...
sys_exofork(void)
{
	...
	retval = env_alloc(&newenv, curenv->env_id);
	if (retval < 0) {
		return retval;
	}
...
sys_env_set_status(envid_t envid, int status)
{
	...
	struct Env *e = NULL;
	int32_t retval = envid2env(envid, &e, 1);
	if (retval < 0) {
		return retval;
	}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	retval = user_mem_check(e, tf, sizeof(struct Trapframe), PTE_U);
	if (retval < 0) 
		return retval;
	e->env_tf = *tf;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	retval = page_insert(e->env_pgdir, pi, va, perm);
	if (retval < 0) {
		page_free(pi);
		return retval;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int32_t retval = envid2env(srcenvid, &srcenv, 1);
	if (retval < 0) {
		return retval;
	}
	struct Env *dstenv = NULL;
	retval = envid2env(dstenvid, &dstenv, 1);
	if (retval < 0) {
		return retval;
	}
	pte_t *entry = NULL;
	struct PageInfo *pi = page_lookup(srcenv->env_pgdir, srcva, &entry);
	if ( (!(*entry & PTE_W))
	     && (perm & PTE_W)
	) {
		return -E_INVAL;
	}
	retval = page_insert(dstenv->env_pgdir, pi, dstva, perm);
	if (retval < 0) {
		return retval;
	}
	return 0;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	struct Env *env = NULL;
	int32_t retval = envid2env(envid, &env, 0);
	if (retval < 0) {
		return retval;
	}
	...
	if (va < UTOP && (perm & ~PTE_SYSCALL) != 0) {
		return -E_INVAL;
	}

	int32_t received_perm = 0;
	if (va < UTOP) {
		pte_t *entry = NULL;
		...
		retval = page_insert(env->env_pgdir, p, env->env_ipc_dstva, perm);
		if (retval < 0) {
			return retval;
		}
		received_perm = perm;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	if (syscallno == SYS_cputs) {
		user_mem_assert(curenv, (char*)a1, a2, PTE_U);
